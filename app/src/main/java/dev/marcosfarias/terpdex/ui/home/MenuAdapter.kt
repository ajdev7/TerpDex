package dev.marcosfarias.terpdex.ui.home
...
import dev.marcosfarias.terpdex.model.Menu
...
class MenuAdapter(
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindView(item: Menu) {
            ...
            val color = TerpmonColorUtil(itemView.context).convertColor(item.color)
            itemView.relativeLayoutBackground.background.colorFilter =
                PorterDuffColorFilter(color, PorterDuff.Mode.SRC_ATOP)

            itemView.setOnClickListener {
                it.findNavController().navigate(R.id.action_navigation_home_to_navigation_terpdex)
            }
        }
    }
